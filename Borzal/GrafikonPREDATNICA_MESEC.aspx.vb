...
Partial Class GrafikonPREDATNICA_MESEC
    ...
    Public Shared Function GetChartData() As List(Of Object)
        Dim query As String = "select SUM(KOLICINA) as ukupno, DATENAME(month,DATUM) AS MESEC  from vpredatnica GROUP BY DATENAME(month,DATUM) order by ukupno desc"
        Dim constr As String = ConfigurationManager.ConnectionStrings("BORZALConnectionString").ConnectionString
        Dim chartData As New List(Of Object)()
        chartData.Add(New Object() {"MESEC", "UKUPNO"})
        Using con As New SqlConnection(constr)
            Using cmd As New SqlCommand(query)
                ...
                Using sdr As SqlDataReader = cmd.ExecuteReader()
                    While sdr.Read()
                        chartData.Add(New Object() {sdr("MESEC"), sdr("UKUPNO")})
                    End While
                End Using
                con.Close()
                Return chartData
