...
Partial Class Sirovine_Pretraga
...
#Region "GENERISANJE MB"
    ...
    Private Sub PopuniTabeluMB()
        ...
        MBListaTitle.InnerHtml = "Spisak sirovina"
    ...
    Protected Sub UslovWHERE()
        ...
        If NAZIV <> "" And NAZIV <> "0" Then
            If WhereSQL = "" Then
                WhereSQL = "(NAZIV = '" & NAZIV & "')"
            Else
                WhereSQL = WhereSQL & " AND (NAZIV = '" & NAZIV & "')"
            End If
        End If
...
#Region "COMBO PRETRAGA"
    Private Sub PopuniCOMBO()
        ...
        Dim UpitNAZIV As String = "SELECT DISTINCT NAZIV FROM SIROVINE ORDER BY NAZIV"
        PopuniDDL_1KOLONA(DDLNAZIV, UpitNAZIV, "NAZIV")
        DDLNAZIV.Items.Insert(0, New ListItem("- - Izaberite naziv sirovine - -", "0"))
    ...
    Protected Sub DDLNAZIV_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DDLNAZIV.SelectedIndexChanged
        Me.PopuniTabeluMB()
    End Sub
#End Region
...
#Region "EXCEL EXPORT"
    Protected Sub EXCEL_Grid(ByVal ID As Integer)
        ...
        Dim IzvestajNaziv As String = "SIROVINE" & "_" & DateTime.Now.ToString("yyyyMMdd_HHmm") & ".xls"
        ...
        If Me.gvExcel.Rows.Count > 0 Then
            ...
            Using sw As New StringWriter()
                ...
                Me.gvExcel.Caption = "<h4>" & "SIROVINE_" & "_" & DateTime.Now.ToString("dd.MM.yyyy.") & "</h4>"
